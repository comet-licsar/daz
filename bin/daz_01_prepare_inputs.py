...
Note, if you want to process only a selected frame(s), and extract accurate epoch times (slow procedure), check params of
daz_lib_licsar.extract_all2txt(outfr='frames.txt', outdaz='esds.txt', inframelist=None, fix_epoch_time=False)
...
def main(argv=None):
    ...
    # processing itself:
    print('Generating output frames csv file - ETA about an hour as we derive lot of info from frame txt files etc.')
